...
\end{theorem}
\begin{proof} 
We know that for all \(x \ge 0\), \(x = \abs{x}\) and for all \(x < 0\), \(x < \abs{x}\), hence \(x \le \abs{x}\). And given a real number \(x\), \(\abs{x} = \sqrt{x^2}\).
Then
	\begin{equation}
		\begin{aligned}
		\abs{a+b}^2 = (a+b)^2 &= a^2 + 2ab + b^2 \\
		&= \abs{a}^2 + 2ab + \abs{b}^2 \\
		&\le \abs{a}^2 + 2\abs{a}\abs{b} + \abs{b}^2 && ab \le \abs{ab} \\
		&= (\abs{a} + \abs{b})^2 \\
		\end{aligned}
	\end{equation}
...
\end{definition}
\section{Graphs} 
The number \abs{a-b} is the distance between \(a\) and \(b\). Then the set of numbers \(x\) which satisfy \(\abs{x-a}< \varepsilon\) may be pictured as the collection of points whose distance from \(a\) is less than \varepsilon 
\[
...
The function \textbf{f approaches the limit l near a} means: for all \(\varepsilon > 0\) there exist \(\delta > 0\) such that, for all \(x\), if \(0 < \abs{x-a} < \delta\), then \(\abs{f(x)-l} < \varepsilon\). 
\end{definition}
\noindent We negate the above by saying
\begin{description}
	\item there exist \(\varepsilon > 0\) such that for all \(\delta > 0\) there exist \(x\) which satisfies  \(0 < \abs{x-a} < \delta\) but not \(\abs{f(x)-l} < \varepsilon\). 
\end{description}
\begin{theorem}
